...
def gemini_insight_handler():
    ...
    try:
        ...
        if cube_type == '3x3':
            try:
                # pycuber expects a specific format for scramble string, ensure it's clean
                # Kociemba's solve() function typically returns a string of moves.
                cube = pc.Cube()
                cube(scramble) # Apply the scramble to the cube
                optimal_solution = str(cube.solve())
        ...
        # MODIFIED: Instructions for optimalSolution and personalizedTip
        prompt = f"""
